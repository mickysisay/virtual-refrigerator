...
it("clicking on Update on rename modal calls backendAPI", () => {
  const refrigerator = { ...REFRIGERATOR_INFO, isOwner: true };
  backendAPI.updateRefrigerator = jest.fn(() =>
    Promise.resolve([{ ...refrigerator, refrigerator_name: "something2" }])
  );
  const updateRef = jest.fn();
  const refrigeratorCard = render(
    <RefrigeratorCard updateRef={updateRef} refrigerator={refrigerator} />
  );
  const editButton = refrigeratorCard.queryByText("Rename");
  userEvent.click(editButton);
...
it("clicking on Delete should open delete modal", () => {
  ...
  expect(
    refrigeratorCard.queryByText(
      `Are you sure you want to delete "${refrigerator.refrigerator_name}"?`
    )
  ).toBeTruthy();
...
it("clicking on delete on delete modal calls backendAPI", () => {
  const refrigerator = { ...REFRIGERATOR_INFO, isOwner: true };
  backendAPI.deleteRefrigerator = jest.fn(() =>
    Promise.resolve([{ ...refrigerator, refrigerator_name: "something2" }])
  );
  const updateRef = jest.fn();
  const refrigeratorCard = render(
  ...
  userEvent.click(deleteButton);
  const confirmDelete = refrigeratorCard.queryByTestId(
    "confirm-delete-refrigerator"
  );
...
it("clicking on refrigerator card calls handleClick prop", () => {
  ...
  const refrigeratorCard = render(
    <RefrigeratorCard handleClick={handleClick} refrigerator={refrigerator} />
  );
